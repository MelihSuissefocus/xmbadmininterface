...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      // NOTE: Here we ensure err.message is captured!
      return NextResponse.json(
        { error: "PDF_RENDER_FAILED", details: err instanceof Error ? err.message : String(err) },
        { status: 500 },
      );
    }
    ...
    console.error("Critical API Crash in CV Generator:", globalError);
    return NextResponse.json(
      {
